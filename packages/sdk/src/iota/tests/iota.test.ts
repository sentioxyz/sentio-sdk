...
import { IotaObjectProcessor, IotaObjectTypeProcessor } from '../iota-object-processor.js'
import { MoveOwnerType } from '@sentio/protos'
import { BUILTIN_TYPES } from '@typemove/move'

describe('Test Iota Example', () => {
  const service = new TestProcessorServer(async () => {
    ...
    IotaObjectProcessor.bind({ objectId: '0x56a' }).onTimeInterval((self, objects, ctx) => {
      ctx.meter.Gauge('size').record(objects.length)
    })

    IotaObjectTypeProcessor.bind({
      objectType: dynamic_field.Field.type(BUILTIN_TYPES.U64_TYPE, validator.ValidatorV1.type())
    })
      .onTimeInterval(
        (self, objects, ctx) => {
          ctx.meter

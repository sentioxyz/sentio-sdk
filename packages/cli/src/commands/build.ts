...
import { getABIFilePath, getABI, writeABIFile, convertLegacySuiAbis } from '../abi.js'
...
export async function codegen(genExample: boolean) {
  ...
  // Convert any Sui ABIs saved by an older CLI in the legacy JSON-RPC shape;
  // the current codegen only reads the gRPC array shape, so a stale file left
  // in abis/sui would otherwise make the Sui codegen throw for the whole
  // project.
  convertLegacySuiAbis(path.resolve('abis', 'sui'))

...
- **Build & bundling (tsdown)**: Packages and user processors bundle with **tsdown** (rolldown/oxc) — not tsup/esbuild — and the build output dir is `dist/` (not `lib/`). tsdown configs live at `packages/*/src/*.config.ts`, **except** the sdk/action processor configs at `src/tsdown.config.js` — these are intentionally **plain JS, not TS**, and are copied verbatim to `dist/` to be run by `sentio build` from the user's `node_modules`. They must stay `.js`: Node refuses to strip types from `.ts` files under `node_modules` (`ERR_UNSUPPORTED_NODE_MODULES_TYPE_STRIPPING`), so a `.ts` config there fails to load, whereas a `.js` config loads natively on every Node version and package manager. (`packages/runtime/src/tsdown.config.ts` stays `.ts` — it bundles the runtime *in-repo*, never under `node_modules`.) The CLI also enforces that `@sentio/cli` and `@sentio/sdk` share the same **major** version before building (`checkSdkCompatibility` in `packages/cli/src/utils.ts`). Non-obvious gotchas: configs set `cwd: process.cwd()` because tsdown resolves entries relative to the **config file's** dir; `fixedExtension: false` emits `.js`/`.d.ts` per package `type`; `outputOptions.codeSplitting: false` keeps the single-entry processor build to exactly one `dist/lib.js`; a `sentio-registration-guard` plugin marks every `src/**/*.ts` `moduleSideEffects: 'no-treeshake'` so self-registering handlers survive tree-shaking; `deps.alwaysBundle: [/.*/]` inlines the user's deps into the self-contained bundle. The `sentio` bin is a committed `packages/cli/bin.mjs` launcher because the built `dist/index.js` doesn't exist at `pnpm install` time (pnpm can't link a bin whose target is absent).
...
- **`packages/runtime` must not import from `packages/sdk`**: the dependency runs sdk → runtime. Anything the runtime needs from the sdk side has to arrive through a structural contract, the reported config, or a value the sdk passes in — not an import.
- **A process's final result must be the last thing it puts on its stream.** `DataBindingContext.finish()` flips the closed flag and must be called in the *same synchronous step* that emits the result (see `service-v3.ts` `startProcess`), never from a later `.finally()`: that gap is several microtasks wide, and a detached continuation slipping a store op through it writes to a stream the driver may have already recycled — which fails an unrelated process with driver `ERR200 "unexpected ProcessID"`. The guards in `db-context.ts` (`sendRequest` rejects, `sendTimeseriesRequest`/`sendTemplateRequest` drop) only work because the flag is set that early.
- **Diagnostics users must act on belong in the error's `message`**: the datasource log view renders only `message`, so a stack left in `err.stack` is invisible to the person who has to fix it — inline it, and bound it, because a diagnostic's length must never scale with the payload it describes. Naming *which* handler needs `PluginManager.describeBinding` (labels derived from the reported config, matching the driver's own `controller.HandlerID`), not the stack: work that detached from a handler — an un-awaited `Promise.all` branch, say — no longer has the handler frame on its stack at all.

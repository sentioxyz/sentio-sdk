...
- **Diagnostics users must act on belong in the error's `message`**: the datasource log view renders only `message`, so a stack left in `err.stack` is invisible to the person who has to fix it — inline it, and bound it, because a diagnostic's length must never scale with the payload it describes. Note also that a late call's stack no longer contains the handler frame: work that detached from a handler — an un-awaited `Promise.all` branch, say — has lost it, so the stack shows where the call was made from, not which handler it belonged to.
